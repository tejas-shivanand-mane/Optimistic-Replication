...
int main(int argc, char *argv[])
{
  ...
  std::vector<std::unordered_set<std::string>> addproject(nr_procs), addemployee(nr_procs);
  ...
  {
    addproject[i - 1].insert(std::to_string(1000 * i));
    real_num_calls++;
    addemployee[i - 1].insert("1");
    real_num_calls++;
    ...
    for (auto it = calls[i].begin() + 3; it != calls[i].end(); ++it)
    {
      ...
      if (call != "5")
      {
        if (call.substr(0, 2) == "0 ")
        {
          ...
          addproject[i].insert(c_id);
        }
        else if (call.substr(0, 2) == "1 ")
        {
          std::string c_id = call.substr(2);
          addproject[i].erase(c_id);
        }
        else if (call.substr(0, 2) == "2 ")
        {
          std::string e_id = getRandomElement(addemployee[i]);
          std::string p_id = getRandomElement(addproject[i]);
          *it = "2 " + e_id + "-" + p_id;
        }
        else if (call.substr(0, 2) == "3 ")
        {
          std::string c_id = call.substr(2);
          addemployee[i].insert(c_id);
        }
        else if (call.substr(0, 2) == "4 ")
        {
          std::string c_id = call.substr(2);
          addemployee[i].erase(c_id);
        }
      }
    }
  }
