...
public:
  ...
  int timestamp;
  // Default constructor
  RegState() : index(0), local_value(0), local_timestamp(0), value(0), timestamp(0){}
};
...
public:
  ...
  void updateLocal(Call &call)
  {
    if (call.type == "Update")
    {
      reg_state.local_timestamp += 1;
      reg_state.local_value = call.value1;
      call.value2 = reg_state.local_timestamp;
      std::cout << "Local Reg: " << reg_state.local_value << std::endl;
      waittobestable.store(waittobestable.load() + 1);
    }
    else if (call.type == "Read")
    {
      if (reg_state.local_timestamp > reg_state.timestamp)
      {
        reg_state.value = reg_state.local_value;
        reg_state.timestamp = reg_state.local_timestamp;
      }
    }
    else
    {
  ...
  void updateRemote(Call call)
  {
    if (call.type == "Update")
    {
      if (reg_state.timestamp < call.value2)
      {
        reg_state.timestamp = call.value2;
        reg_state.value = call.value1;
      }
      std::cout << "Remote Reg: " << reg_state.value << "time stamp" << reg_state.timestamp << std::endl;
      waittobestable.store(waittobestable.load() + 1);
    }
    else
    {
