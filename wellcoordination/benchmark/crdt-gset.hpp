...
public:
  int index;
  std::set<int> localelements;
  std::set<int> remoteelements;
...
public:
  ...
  {
    switch (static_cast<GsetEnum>(set))
    {
    case Update:
      return "Update";
    case Read:
  ...
  void updateLocal(Call &call)
  {
    if (call.type == "Update")
    {
      current_state.localelements.insert(call.value1);
      std::cout << "Local Gset: " << call.value1<< std::endl;
  ...
  void updateRemote(Call call)
  {
    if (call.type == "Update")
    {
      current_state.remoteelements.insert(call.value1);
      std::cout << "Remote Gset: " << call.value1 << std::endl;
      waittobestable.store(waittobestable.load() + 1);
    }
