...
int main(int argc, char *argv[])
{
  ...
  std::string loc = "/scratch/user/u.js213354/Optimistic-Replication/wellcoordination/workload/";
  ...
  std::string callStraddp, callStrdeletep, callStradde;
  //std::vector<std::unordered_set<std::string>> elements(nr_procs);
  //callStraddp = std::string("0 ") + "1";
  //callStradde = std::string("3 ") + "1";
  /*for (int i = 1; i <= nr_procs; i++)
  {
      calls[i - 1].push_back(callStraddp);
      calls[i - 1].push_back(callStradde);
  }*/
  for (int i = 1; i <= nr_procs; i++)
  {
    //elements[i-1].insert(std::to_string(1000 * i));
    //real_num_calls++;
    for (int type = 0; type <= 0; type++)
    {
      ...
      for (; count < expected_calls_per_update_method;)
      {
        ...
        if (type == 0)
        {
          std::string c_id = std::to_string(1+ std::rand() % 1000);
          callStr = "0 " + c_id;
          real_num_calls++;
        }
  ...
  for (int i = 0; i < nr_procs; i++)
  {
    calls[i].insert(calls[i].begin(),
                    std::string("#" + std::to_string(real_num_calls)));
