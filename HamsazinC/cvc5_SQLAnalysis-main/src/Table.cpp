...
Table::Table(const Table& other)
        : slv(other.slv), tableSort(other.tableSort), tableName(other.tableName), columnNames(other.columnNames) {
    // All members have been copied
}

Table& Table::operator=(const Table& other) {
    if (this != &other) {
        // Copy other members
        tableSort = other.tableSort;
...
//    tableSort = slv->mkBagSort(tupleSort);
    tableSort = slv->mkSetSort(tupleSort);
}
...
//    DatatypeConstructor tableCons = tableSort.getBagElementSort().getDatatype()[0];
    DatatypeConstructor tableCons = tableSort.getSetElementSort().getDatatype()[0];
...
vector<uint32_t> Table::getIndicesOfColumns(const vector<std::string>& desiredColumnNames) {
    vector<uint32_t> indices;
    // Iterate through each desired column name
    for (const auto& desiredName : desiredColumnNames) {
        // Search in columnNames for a pair whose first element matches desiredName
        for (uint32_t i = 0; i < columnNames.size(); ++i) {
            if (columnNames[i].first == desiredName) {
                indices.push_back(i);
                break;
            }
        }
...
vector<string> Table::getTableColumnNames() {
    vector<string> tableColumnNames;
    for (const auto& entry : columnNames) {
        tableColumnNames.push_back(entry.first);
    }
...
Term Table::createEmptyBag() {
//    Sort addressTableSet = slv->mkBagSort(tableSort.getBagElementSort());
//    Term emptyAddressTable = slv->mkEmptyBag(addressTableSet);
    Term emptyAddressTable = slv->mkEmptySet(tableSort);
    return emptyAddressTable;
}

Term Table::createDefaultBag() {
    RowData defaultValues;
    for(auto& columnName : columnNames) {
        if(columnName.second == slv->getIntegerSort()) {
            defaultValues.emplace_back(0);
        }
        else if(columnName.second == slv->getStringSort()) {
            defaultValues.emplace_back("");
        }
    }
    Term defaultRow = this->createRow(defaultValues);
    return defaultRow;
}
...
//    const Datatype& rowType = row.getSort().getDatatype();
    ...
    try {
        ctor = row.getSort().getDatatype()[0];
    }
    catch (exception& e) {
        int indexColumn = getColumnIndexByName(fieldName);
        const DatatypeSelector& selector = ctor[indexColumn];
        return slv->mkTerm(Kind::APPLY_SELECTOR, {selector.getTerm(), row});
    }
